...
validDir Game::checkValid(Piece *p)
{
	...
	if(p->getStrength()==6||p->getStrength()==7)
	{
		if(cX>0)
...
void Game::click(float x, float y)
{
	...
	}else if(staged)
	{
		...
		if((xCh!=curX||yCh!=curY)&&(((xCh==curX-valid.left||xCh==curX+valid.right)&&yCh==curY)||((yCh==curY-valid.up||yCh==curY+valid.down)&&curX==xCh))){
			if(pieces[yCh][xCh]->getID()!="empty")
			{
				delete pieces[yCh][xCh];
